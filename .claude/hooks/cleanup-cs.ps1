...
#
# Skips itself entirely if the working tree has no modified .cs files.
...
# Modified .cs files (staged + unstaged + untracked).
...
& dotnet format Clet.slnx --no-restore *> $null

# ReSharper code cleanup. --include narrows the work to changed files.
$includes = ($changed | ForEach-Object { "--include=$_" }) -join ' '
if ($includes) {
    & dotnet jb cleanupcode Clet.slnx --profile="Built-in: Full Cleanup" $includes.Split(' ') --no-build *> $null
}

# Surface the net effect so the agent sees its own drift.
$after = & git status --porcelain | Where-Object { $_ -like '* *.cs' }
if ($after) {
